...
The Fitzhugh-Nagumo model is a simplified mathematical representation of the neural dynamics of a neuron. It is essentially a third-order limit-cycle oscillator, which is characterized by the presence of two variables: the membrane potential $V$ and the recovery variable $n$. 

\vspace{12pt}
The FitzHugh-Nagumo model equations are given by:


%\documentclass{article}
%\usepackage{amsmath}
\begin{equation}
    \frac{dV}{dt} = V - \frac{V^3}{3} - n + I_{\text{ext}}
\end{equation}
...
where $V$ is the membrane potential, $n$ is the recovery variable.
$a$,$b$ and \textit{$\epsilon$} are parameters. $I_{\text{ext}}$ is the external current or input to neuron.

\vspace{12pt}

The model exhibits different types of behavior depending on the control parameter $a$. Specifically, the system can display either stable fixed-point behavior or limit cycles. This model can show table fixed-point behavior (Figs. 5.3(a) and (d)) or limit cycles (Figs. 5.3(c) and 3(d)), depending on the value of the control parameter. 
...
The NaK model (Sodium-Potassium model) is a sophisticated mathematical representation that illuminates the intricacies of two-dimensional phase planes. Its intriguing dynamics are contingent upon the bias current $I$, which can result in complex patterns of bistability and bifurcations. 

\begin{equation}
C\frac{dV}{dt} = I - g_{\text{L}}(V - E_{\text{L}}) - g_{\text{Na}}m_{\infty}(V)(V - E_{\text{Na}}) - g_{\text{K}}n(V - E_{\text{K}})
\end{equation}
...
where $C$ is the membrane capacitance, $V$ is the membrane potential, $g_{L}
$ is the leak conductance, $E_{L}$ is the leak reverse potential, $g_{Na}$ is sodium conductance, $E_{Na}$ is sodium reverse potential. $I$ is the applied current, $\tau(V)$ is the time constant, $n_{\infty}(V)$ is the steady-state value of gating variable and $m_{\infty}(V)$ is the steady-state value of sodium channel activation parameter.

\vspace{12pt}
This model is highly useful for delving deeper into the parameter space and comprehending the diverse range of behaviors that it can exhibit. One of the interesting features of the NaK model is its ability to display bistability, which is an important aspect of neurodynamics. 
...
Finally, the membrane potential for a trajectory close to the homoclinic orbit is demonstrated in Fig. 5.6. 
...
During the rapid traversal of the orbit, the membrane potential undergoes a quick change, and this is followed by a slow approach to the saddle point. This slow approach leads to a characteristic spiking appearance of the membrane potential. The spiking pattern is a result of the slow approach to the saddle, which causes the membrane potential to oscillate back and forth \cite{ref10}. These oscillations are responsible for the spiking appearance of the membrane potential.
...
\begin{thebibliography}{9}
    ...
    \bibitem{MysteriesOfTheActionPotential} A. Johnson and W. Winlow, “Mysteries of the action potential,” Physiology News, no. Summer 2018, pp. 38–41, Jul. 2018, doi: https://doi.org/10.36866/pn.111.38.
    \bibitem{SpikeTrains}Păun, G., Pérez–Jiménez, M. J., and Rozenberg, G. (2006). Spike trains in spiking neural p systems. International Journal of Foundations of Computer Science, 17(04), 975-1002. https://doi.org/10.1142/s0129054106004212
    \bibitem{CurrentBiology} Bardwell, L. (2020). Cancer mutations: molecular mekanisms. Current Biology, 30(5), R222-R224. https://doi.org/10.1016/j.cub.2020.01.039
